...
public class TestRequestController {
    ...
    // Extra code added to get the TestRequests with id.
    @PreAuthorize("hasAnyRole('TESTER')" +
                  "|| hasAnyRole('DOCTOR')")
    @GetMapping("/api/testrequests/{id}")
    public Optional<TestRequest> getTestRequestsWithId(@PathVariable Long id) {
        try {
            User user = userLoggedInService.getLoggedInUser();
            Optional<TestRequest> testRequest = null;
            if(user.doesRoleIsTester()){
                testRequest = testRequestQueryService.findByTesterAndID(user, id);
            }
            if(user.doesRoleIsDoctor()){
                testRequest = testRequestQueryService.findByDoctorAndID(user, id);
            }
